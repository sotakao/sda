...
def main():
    ...
    wandb.init(project=args.wandb_project, entity=args.wandb_entity,
                name=f"SDA_filtering_{args.obs_fn}_{args.obs_pct}pct_{args.guidance_method}"
                    f"_strength{args.guidance_strength}_em_steps{args.steps}_corr{args.corrections}",
                config=vars(args))
    
    pred_t  = state.squeeze(1).detach().cpu()                        # (n_ens,C,H,W) in scalefact × PV
    gt_t    = (scalefact * pv_truth[args.window-1].detach().cpu().float())  # (C,H,W)
    wandb.log({
    ...
    for i in range(1, filter_window-args.window+1):
        ic_vec = posterior_samples[:,1].flatten(start_dim=1) # Shape (B, M_ic)
        y_clean = torch.cat([ic_vec,
                             obs_fn(ys[None, i+1:i+args.window].reshape(1,-1).repeat(args.n_ens, 1))
    ...
    for t in range(posterior_time_first.shape[0]):
        ...
        gt_t    = (scalefact * pv_truth[args.window-1 + t].detach().cpu().float())  # (2,H,W)
        if t % args.plot_every == 0:
            fname = save_spectrum(pred_t, gt_t.unsqueeze(0), time=(args.window-1 + t))
            wandb.log({"spectrum": wandb.Image(fname)}, step=(args.window-1 + t))
